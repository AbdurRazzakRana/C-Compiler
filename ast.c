...
int check_params(ASTnode *actuals, ASTnode *formals)
{
  ...
  if (actuals->my_data_type != formals->my_data_type)
    return 0;
  if (actuals->symbol->SubType != formals->symbol->SubType)
    return 0;
...
{
  if (mytype == A_INTTYPE)
  { // if the enum is int type return string INT
    return "INT";
  }
  if (mytype == A_VOIDTYPE)
  { // if the enum is VOID type return string VOID
    return "VOID";
  }
  printf("\n\nBurf: undefined type\n\n");
  return "";
...
void ASTprint(int level, ASTnode *p)
{
  ...
  case A_EXPR_STAT: // Expression Statement
    ...
    if (p->s1 != NULL)
      ASTprint(level + 1, p->s1); // call the expression if it has
  ...
  case A_FUNC_CALL: // case for function call
    ...
    if (p->s1 == NULL)
    { // just to match the print of NULL with LAB 7 video 1 42.27(time) output
      PT(level + 2);
      printf("NULL\n");
    }
    else
      ASTprint(level + 2, p->s1); // arguments
  ...
  case A_RETURN_STAT: // return statement
    ...
    if (p->s1 != NULL)
      ASTprint(level + 1, p->s1); // if there any expression exist, call it
